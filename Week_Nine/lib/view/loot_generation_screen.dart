...
import 'package:dice_dungeons_nine/gradient_background.dart'; // Custom background widget we have been using a lot
...
class _LootGenerationScreenState extends State {
  ...
  Future _loadLootJson() async {
    ...
    setState(() {
      ...
      _lootList = jsonData['tier$_lootTier'] ??
          []; // fallback to empty list, error prevention
    });
  }
  ...
  void changeLootTier(int delta) async {
    ...
    if (newTier >= 1 && newTier <= 3) {
      ...
      final String jsonString =
          await rootBundle.loadString('assets/data/loot.json');
